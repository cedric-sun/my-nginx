...
struct ngx_module_s {
    ...
    // the subscript in `*ngx_modules[]`, generated by `configure` in `objs/ngx_modules.c`
    ngx_uint_t            index;

    // point to the same string in `ngx_module_names`, generated by `configure` in `objs/ngx_modules.c`
    char                 *name;
    ...
    void                 *ctx;
    ngx_command_t        *commands; // commands supported by the current module
    ngx_uint_t            type; // one of `NGX_(CORE|EVENT|HTTP|MAIL|STREAM)_MODULE`

    ngx_int_t           (*init_master)(ngx_log_t *log);

    // called in master process after each succ config parse (including conf reload)
    ngx_int_t           (*init_module)(ngx_cycle_t *cycle);

    // called in each worker process
    ngx_int_t           (*init_process)(ngx_cycle_t *cycle);

    // these 2 are currently unused
    ngx_int_t           (*init_thread)(ngx_cycle_t *cycle);
    void                (*exit_thread)(ngx_cycle_t *cycle);

    // called (in worker process) before each worker process exits
...
// `*ctx` of all `NGX_CORE_MODULE` should point to instances of this structure
typedef struct {
    ngx_str_t             name;
...
extern ngx_module_t  *ngx_modules[]; // `objs/ngx_modules.c`; NULL terminated pointer array
extern ngx_uint_t     ngx_max_module;

extern char          *ngx_module_names[]; // `objs/ngx_modules.c`; NULL terminated pointer array
