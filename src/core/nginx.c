...
// static globals that reflects CLI flags.
// Will be set by `ngx_get_options()`
static ngx_uint_t   ngx_show_help;      // -?   -h
static ngx_uint_t   ngx_show_version;   // -?   -h  -v -V
static ngx_uint_t   ngx_show_configure; // -V
static u_char      *ngx_prefix;     // -p <dir_path>
static u_char      *ngx_error_log;  // -e <file_path>
static u_char      *ngx_conf_file;  // -c <file_path>
static u_char      *ngx_conf_params;  // -g <directive>: set directive in "main" context
static char        *ngx_signal;  // -s (stop|quit|reopen|reload)
...
main(int argc, char *const *argv)
{
    ...
    ngx_debug_init(); // macro that expands to nothing on all OS
    ...
    // defined in `os/unix/ngx_posix_init.c`. Unsure what it means.
